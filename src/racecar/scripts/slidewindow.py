...
class SlideWindow:
    ...
    def slidewindow_(self, img):
        ...
        pts_left = np.array([[width / 2 - 70, height], [width / 2 - 70, height - 60], [width / 2 - 130, height - 80],
                             [width / 2 - 130, height]], np.int32)
        cv2.polylines(out_img, [pts_left], False, (0, 255, 0), 1)
        pts_right = np.array([[width / 2 + 57, height], [width / 2 + 57, height - 80], [width / 2 + 120, height - 110],
                              [width / 2 + 120, height]], np.int32)
        ...
        if line_flag != 3:
            ...
            for i in range(len(good_left_inds)):
                img = cv2.circle(out_img, (nonzerox[good_left_inds[i]], nonzeroy[good_left_inds[i]]), 1, (0, 255, 0),
                                 -1)
            # window sliding and draw
            for window in range(0, nwindows):
                if line_flag == 1:
